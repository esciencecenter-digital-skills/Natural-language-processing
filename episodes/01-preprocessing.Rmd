...
4. Train the model
5. Save the model
6. Load the embeddings and inspect them 
...
Given newspaper corpora [C1, C2, C3, ...] containing texts created in time periods from 1950s to 1990s, considered as four decades [1: 50-60; 2: 60-70; 3: 70-80; 4: 80-90], the task is to locate words with different meaning in different time periods.

We're going to focus on three words: `mobiel`, `televisie` and `ijzeren`. These words exemplify very well the notion of semantic evolution / semantic shift, as their meaning has gained new nuances due to social, technological, political and economic changes occurred in those key years.
...
Now we will train a model to extract word embeddings, these will help us to detect how the meaning of `ijzeren`, `televisie` and `mobiel` have shifted over the years, from the 50s to the 90s. 

A number of publications (e.g., [Turney et al., 2010](https://www.jair.org/index.php/jair/article/view/10640); [Baroni et al., 2014](https://aclanthology.org/P14-1023.pdf)) have showed that `distributional word representations`, also known as `embeddings` provide an efficient way to track how meanings of words change across years. Let's see what are those and how they manage to do that.
...
By representing words as vectors with multiple dimensions, we capture more nuances of their meanings or characteristics. This method allows us to quantify similarities between words, which is essential for tasks like detecting semantic changes over time. While visualizing high-dimensional data is challenging, mathematical tools like cosine similarity enable us to work effectively in these spaces.
...
### Load the Word2Vec Models Dutch Newspapers

We proceed to load our models. We will dynamically read all model files from the folder and organizing them into lists based on their decades. We'll do this with a function called `load_models_by_decade`:


```python
# maybe this function can go somewhere else and be loaded directly as part of a custom library?
# I don't see people writing the whole thing

def load_models_by_decade(folder_path):
    filenames = os.listdir(folder_path)
    models_by_decade = defaultdict(list)
    pattern = re.compile(r'_(\d{4})_(\d{4})\.w2v$')
    ...
    for filename in filenames:
        ...
        if match:
            ...
            decade_key = f"{str(start_year)[2]}0_{str(end_year)[2]}9"
            path = os.path.join(folder_path, filename)
            try:
                model = KeyedVectors.load_word2vec_format(path, binary=True)
                models_by_decade[decade_key].append(model)
...
Next we will use it to load our models:

```python
models_by_decade = load_models_by_decade(folder_path)

```

Let's check if we have all models and whether these are organised into decades:

```python
# Print the organized models
for decade, model_list in models_by_decade.items():
    print(f"{decade}: {model_list}")
```

```python
60_69: [<gensim.models.keyedvectors.KeyedVectors object at 0x164ae04c0>, <gensim.models.keyedvectors.KeyedVectors object at 0x1698d1610>, <gensim.models.keyedvectors.KeyedVectors object at 0x1695e2ee0>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85f10>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85e50>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85cd0>]
80_89: [<gensim.models.keyedvectors.KeyedVectors object at 0x164ad5d90>, <gensim.models.keyedvectors.KeyedVectors object at 0x1698d18e0>, <gensim.models.keyedvectors.KeyedVectors object at 0x1695e2f70>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85ee0>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85e20>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85d00>]
70_79: [<gensim.models.keyedvectors.KeyedVectors object at 0x164ad5fa0>, <gensim.models.keyedvectors.KeyedVectors object at 0x1695e2eb0>, <gensim.models.keyedvectors.KeyedVectors object at 0x164aa2e50>, <gensim.models.keyedvectors.KeyedVectors object at 0x164aa2fa0>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85e80>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85ca0>]
50_59: [<gensim.models.keyedvectors.KeyedVectors object at 0x1698d1280>, <gensim.models.keyedvectors.KeyedVectors object at 0x1698d1fd0>, <gensim.models.keyedvectors.KeyedVectors object at 0x1695e2f10>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85c40>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85ac0>, <gensim.models.keyedvectors.KeyedVectors object at 0x168f85c10>]
```

Let's import first other classes, one `KeyedVectors` from `gensim` allows us to extract embeddings from the model, while `cosine_similarity` from `sklearn` will help us compute the similarity across words.
...
Secondly, we need to average out the embeddings across the decades. The reason for this is that we have many newspapers, and each one of them has separate embeddings for our word. We want our embeddings to reflect all the newspapers considered.

We do this with a function, `get_average_embeddings`:

```python
def get_average_embeddings(word, models_by_decade):
    embeddings = {}
    for decade, models in models_by_decade.items():
        vectors = [model[word] for model in models if word in model]
        if vectors:
            embeddings[decade] = np.mean(vectors, axis=0)
        else:
            embeddings[decade] = None
            print(f"'{word}' not found in any models for decade {decade}, skipping.")
    return embeddings
```

Then to look at how these
