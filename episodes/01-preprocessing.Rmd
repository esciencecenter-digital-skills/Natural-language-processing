...
To do so we need a dataset large enough to allow our model to learn meaningful associations between a particular set of letters and their meaning. These associations are crystallized into vectors which in the NLP domain are referred to as `embeddings`. We will go into details about those when we actually create them.
...
We need to install `en_core_web_sm` because the text we're dealing with it's in English. This is a small pre-trained language model containing essential components like vocabulary, syntax, and entities specifically for the English language.
